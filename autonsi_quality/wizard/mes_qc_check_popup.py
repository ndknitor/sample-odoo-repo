...
class MESQualityCheckPopup(models.TransientModel):
    _name = 'mes.qualitycheckpopup'
    _description = 'popup for quality check'

    qc_check_information = fields.Char('QC Check Information')
    mo_id = fields.Many2one('mrp.production', 'MO ID')
    question_ids = fields.Many2one('mes.qc_form_questions', 'Question ID', compute='compute_question_ids')
